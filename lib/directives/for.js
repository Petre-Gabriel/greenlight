...
directive("for", (el, ElementController) => {
  ...
  function updateContent(key) {
    if (
      !variableToIterate.startsWith(key) &&
      !key.startsWith(variableToIterate)
    )
      return;

    const toIterateFromStore = getScopedPropertyFromElement(
      el,
      variableToIterate
    );
  ...
  window.GreenLight.$globalStore.onChange((key) => updateContent(`$${key}`));
