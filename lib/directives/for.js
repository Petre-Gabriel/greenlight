import { directive, setDirectiveData } from "../directive";
 ...
 */
directive("for", (el, ElementController, { getDirectiveAttr }) => {
  // Only accept template elements.
  if (el instanceof HTMLTemplateElement === false) return;

  const forStructure = getDirectiveAttr();
  ...
  const shouldElementReactTo = createReactivityFunction(variableToIterate);

  async function updateContent(key) {
    if (!shouldElementReactTo(key)) return;

    const toIterateFromStore = await evaluateExpressionForElement(
      el,
      variableToIterate
    );
  ...
  window.GreenLight.$globalStore.onChange((key) => updateContent(`$g.${key}`));
